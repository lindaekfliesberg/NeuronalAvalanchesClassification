...
for f in freqbands: # the code iterates over each frequency band
    ...
    for subject in tqdm(subject_select, desc="subject"): # the code iterates over each subject in the list subject_select
        ...
        for kk_min_samples in min_samples_leaf_test:
            pipeline["avv"+"-RF_test"] = Pipeline(steps=[('gav', gav), ('rf', RandomForestClassifier(n_estimators=80, min_samples_leaf=kk_min_samples))])
...
dataset_average_grouped = (dataset_av.groupby(["n_leafs"])[["score", "score_train"]].agg(["mean", "std"]).reset_index())
dataset_average_grouped.columns = ['_'.join(col).strip() for col in dataset_average_grouped.columns.values]

plt.close('all')
line1, = plt.plot(dataset_average_grouped["n_leafs_"], dataset_average_grouped["score_train_mean"], color='b', label="Train AUC")
line2, = plt.plot(dataset_average_grouped["n_leafs_"], dataset_average_grouped["score_mean"], color='r', label="Test AUC")
