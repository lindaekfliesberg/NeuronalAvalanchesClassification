...
dataset_av.to_csv(df_path+"av_LDA_SVC_RF_LR.csv")
...
==========================================================================
"""
## statistical analysis (MOABB)
statistics = compute_dataset_statistics(concat_fc_av) # based on Wilcoxon tests, takes the dataframe as input and returns a table of p values for each pipeline

# plotting
fig = moabb_plt.meta_analysis_plot(statistics, "av"+"-LDA", "av"+"-SVC")
plt.show()

# comparing different classifiers (makes more sense if I have more pipelines with different classifiers)
P, T = find_significant_differences(statistics)
...
dataset_average = (concat_CSP_fc_av_avn.groupby(["subject", "pipeline"])[["accuracy", "score", "kappa"]].agg(["mean", "std"]).reset_index())
...
pipeline_order = ["CSP-LDA", "plv-LDA", "plv-SVC", "av-LDA", "av-SVC", "av-RF", "av-LR", "avn-LDA", "avn-SVC", "avn-RF", "avn-LR", "av-opt-LDA", "av-opt-SVC", "av-opt-RF", "av-opt-LR", "avn-opt-LDA", "avn-opt-SVC", "avn-opt-RF", "avn-opt-LR"]
